...
def run_test_largest_negative_number():
    ...
    # Test 2:
    expected = -5
    answer = largest_negative_number(([], [-1111111111111111], [-5, 3,2]))
    print('Expected and actual are:', expected, answer)
    ...
    # Test 4:
    expected = None
    answer = largest_negative_number(([4, 6, 3, 5, 23], [4, 5, 4, 2, 1, 4], []))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for j in range(len(seq_seq)):
        for k in range(len(seq_seq[j])):
            if seq_seq[j][k] < 0:
                greatest =seq_seq[j][k]
    if greatest == 'a':
        return None
    for seq in seq_seq:
        for num in seq:
            if num > greatest and num<=0:
                greatest = num
    return greatest
